...
_Why:_ keeping business logic out of `infrastructure/` and `views/` is what lets `core/` be unit-tested without a DOM or network.
...
_Why:_ the event-based `createReducer` + passive-action pattern is established project-wide; imperative actions fragment the store's mental model and block refactors.
