...
import { HttpArchiveSiteService } from "../infrastructure/HttpArchiveSiteService";
...
function ArchiveSiteDialog({ siteId, siteName, onSuccess, dialogId }: Props) {
  ...
  const onConfirm = async () => {
    ...
    try {
      await new HttpArchiveSiteService().archive(siteId);
      setArchiveState("success");
    } catch (err) {
      console.error("Impossible d'archiver le site", err);
        ...
        <DsfrDialogTitle>
          {archiveState === "success" ? (
            <>
              ...
              <span>Votre site «&nbsp;{siteName}&nbsp;» a été supprimée</span>
            </>
          ) : (
            <>Voulez-vous vraiment supprimer le site «&nbsp;{siteName}&nbsp;»&nbsp;?</>
          )}
        </DsfrDialogTitle>
        {archiveState === "idle" && <>Les projets associés seront également supprimés.</>}
        ...
        <DsfrDialogFooter
          buttons={[
            ...
            },
            {
              ...
              onClick: onConfirm,
              children: archiveState === "error" ? "Supprimer le site" : "Confirmer la suppression",
              disabled: archiveState === "loading",
            },
          ]}
        ></DsfrDialogFooter>
...
export default ArchiveSiteDialog;
