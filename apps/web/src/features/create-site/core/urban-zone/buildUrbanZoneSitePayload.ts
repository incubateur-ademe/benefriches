...
export type UrbanZoneSiteDataForSave = {
...
/**
 * Extracted (ticket 11) so create and update share one payload builder — the update flow's
 * `siteUpdateSaved` (features/update-site/core/updateSite.actions.ts) calls this unchanged,
 * against `state.siteUpdate.urbanZone.steps` instead of `state.siteCreation.urbanZone.steps`.
 */
export const buildUrbanZoneSiteDataForSave = (
