...
const AvoidedTrafficAccidentsDeathsContent = ({ withMonetarisation }: Props) => {
  return (
    <>
      ...
      <ul>
        <li>
          Répartition des routes empruntées par les véhicules utilisés pour les déplacements
          supprimés (selon le nombre de voies / le trafic)
        </li>
        <li>Densité de population communale</li>
        {withMonetarisation && (
          <li>Valeur du coût de la vie “statistique” (exprimée en €/unité)</li>
        )}

        <li>Nombre de décès par accident (exprimé en unité/100accidents)</li>
        <li>
      ...
      <ul>
        ...
        <li>
          un taux de décès par accident (exprimé en nombre de décès pour 100 accidents), qui est
      ...
      <p>Le nombre de décès évités est égal au produit entre ces 3 paramètres. </p>
      {withMonetarisation && (
        <p>
          Le calcul économique consiste à multiplier ce nombre de décès à la valeur du coût de la
          vie “statistique” (exprimée en €/unité).
        </p>
      )}
...
export default AvoidedTrafficAccidentsDeathsContent;
