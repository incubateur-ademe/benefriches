...
type CascadingChange<StepId extends string> = {
  action: "delete" | "invalidate" | "recompute";
  stepId: StepId;
};

type Props<StepId extends string> = {
  cascadingChanges: CascadingChange<StepId>[] | undefined;
  showAlert: boolean;
...
// Presentational, wizard-agnostic cascading-update confirmation dialog. Each wizard (urban,
// renewable-energy) supplies its own pending state, confirm/cancel dispatchers and step-label
// resolver so this component can render both without knowing which Redux slice it reads from.
export default function CascadingChangesAlertDialog<StepId extends string>({
  cascadingChanges,
  showAlert,
  getStepLabel,
  onConfirm,
  ...
  return (
    <Dialog open={showAlert} onClose={onCancel}>
          ...
          <div className="fr-modal__footer">
            <ButtonsGroup
              ...
              buttons={[
                ...
                { children: validateTextButton, onClick: onConfirm },
              ]}
            />
          </div>
        </DialogPanel>
