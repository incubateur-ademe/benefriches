...
describe("SqlSiteRepository integration", () => {
  ...
  describe("save", () => {
    ...
    it("Saves given site with expenses and incomes in sites, expenses and incomes", async () => {
      ...
      const expensesResult = await sqlConnection("site_expenses").select("amount", "type");
      expect(expensesResult).toEqual([{ amount: 45000.0, type: "other" }]);

      const incomesResult = await sqlConnection("site_incomes").select("amount", "type");
      expect(incomesResult).toEqual([
        { amount: 20000.0, type: "operations" },
        { amount: 32740.3, type: "other" },
      ]);
    });
